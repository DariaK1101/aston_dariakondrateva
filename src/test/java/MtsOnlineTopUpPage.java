...
@Feature("Проверка страницы оплаты")
public class MtsOnlineTopUpPage {
    ...
    @Step("Принять куки, если кнопка доступна")
    ...
    @Step("Прокрутка до формы 'Услуги связи'")
    public void scrollToConnectionForm() {
        ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);",
                driver.findElement(By.id("pay-connection")));
    }

    @Step("Заполнение формы 'Услуги связи' — телефон: {phone}, сумма: {amount}")
    ...
    @Step("Проверка плейсхолдеров и валидаций во всех вкладках")
    ...
    @Step("Проверка окна подтверждения оплаты для номера {phone} и суммы {amount}")
